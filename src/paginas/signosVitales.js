 ...
 const location = useLocation();
  ...
  const buscarSignos = async () => { 
    const estyCons =  await fetch(
      ...
      .then((r) => r.json())
      .then(function (data) {
        if (data === "No hay signos ingresados.") {
            console.log("entro");
            setSignos([]);
            console.log(signos);
            setOpen(false);
  ...
  if (signos === null || signos === "undefined") {
    return (
      <>
        {navSeccion}
        <Box
            ...
            <AccordionDetails>
              <ul>
                <li>
                  ...
                  <Typography>
                  Presión Mínima: {e.PresionMinima !== 0 ? e.PresionMinima : "-"}
                  </Typography>
                </li>
                <li>
                  <Typography>Presión Máxima: {e.PresionMaxima !== 0 ? e.PresionMaxima : "-"} </Typography>
                </li>
                <li>
